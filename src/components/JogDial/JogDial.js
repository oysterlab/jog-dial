...
// https://codepen.io/mysisi/pen/xYNWNZ
// 
// 
...
class JogDial { 
  constructor(canvas, {tickCount, tickLengthRatio, radiusRatio}, initIdx) {
    ...
    const renderer = new THREE.WebGLRenderer({canvas, antialias: true, precision: 'high'})
    ...
    const tickWidth = 0.32 * 10
    const tickHeight = 6.6 * 1.4
    const degRange = Math.PI * 0.3
  ...
  nextTick(dir, duration, isForce) {
    ...
    this.tickAnim = TweenMax.to(anim, duration, {
      ...
      onUpdate: function() {
        ...
        const posY = currTick.init.position.y// +  anim.progress

        const scaleX = currTick.init.scale.x + 0.5 * anim.progress
        const scaleY = currTick.init.scale.y + 1.5 * anim.progress

        currTick.mesh.position.y = posY
        currTick.mesh.position.z = posZ
        currTick.mesh.scale.x = scaleX
        currTick.mesh.scale.y = scaleY

        currTick.curr.position.y = posY
