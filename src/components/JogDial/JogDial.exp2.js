      ...
      const tickMaterial = new THREE.ShaderMaterial({
        ...
        fragmentShader: `
          ...
          void main() {
            ...
            // float d1 = smoothstep(0.0, 1.5, uv.x);
            float horizontalBlur = (uv.x < 0.5) ? smoothstep(-0.5, 1.5, uv.x * 2.0) : smoothstep(1.5, -0.5, (uv.x - 0.5) * 2.);            
            float verticalBlur = smoothstep(0.0 ,0.2, 1. - uv.y) * smoothstep(0.0 ,0.2, uv.y);
  
            float w = smoothstep(0.4, 0.5, uv.x) - smoothstep(0.5, 0.6, uv.x);
            float h = smoothstep(0.1, 0.3, uv.y) - smoothstep(0.7, 0.9, uv.y);
            d = w * h;
            d += horizontalBlur * verticalBlur;
